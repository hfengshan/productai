...
/**
 * @author hanfengshan
 * @abstract The main purpose of this program is to define the multicurl class
 * @version v1.0 2016-1-13
 * @copyright Copyright (c) 2015-2016 Qihoo Data Operation Group  (http://yingxiao.360.cn)
 * @category API - MultiCurl
 */
...
class MultiCurl
{
     ...
     * @access public
     */
    public function multiExec($callback=null)
    {
        if ($this->isStarted) {
            return;
        }
        ...
        do {
            while (($execrun = curl_multi_exec($this->multiCurl, $active)) == CURLM_CALL_MULTI_PERFORM) ;
                    ...
                    foreach ($this->activeCurls as $key => $ch) {
                        if ($ch->curl === $info_array['handle']) {
                            ...
                            if (empty($callback)) {
                                ...
                                call_user_func($this->responseCallback, $ch->exec($ch->curl));
                            }
            ...
            if ($active) {
                curl_multi_select($this->multiCurl, $this->timeout);
            }
